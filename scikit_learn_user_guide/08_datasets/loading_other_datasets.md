...
[![../_images/loading_other_datasets-1.png](../_images/loading_other_datasets-1.png) ](<../_images/loading_other_datasets-1.png>)
...
Related links
  ...
  * `Faster API-compatible implementation`: [mblondel/svmlight-loader](<https://github.com/mblondel/svmlight-loader>)
...
The `sklearn.datasets` package is able to download datasets from the repository using the function [`sklearn.datasets.fetch_openml`](<../modules/generated/sklearn.datasets.fetch_openml.html#sklearn.datasets.fetch_openml> "sklearn.datasets.fetch_openml").
...
References
  ...
  * [Vanschoren, van Rijn, Bischl and Torgo. “OpenML: networked science in machine learning” ACM SIGKDD Explorations Newsletter, 15(2), 49-60, 2014.](<https://arxiv.org/abs/1407.7722>)
...
The `"pandas"` and `"liac-arff"` parsers can lead to different data types in the output. The notable differences are the following:
  ...
  * The `"liac-arff"` parser uses float64 to encode numerical features tagged as ‘REAL’ and ‘NUMERICAL’ in the metadata. The `"pandas"` parser instead infers if these numerical features correspond to integers and uses pandas’ Integer extension dtype.
...
Here are some recommended ways to load standard columnar data into a format usable by scikit-learn:
  ...
  * [scipy.io](<https://docs.scipy.org/doc/scipy/reference/io.html>) specializes in binary formats often used in scientific computing contexts such as .mat and .arff
  ...
  * scikit-learn’s [`load_svmlight_file`](<../modules/generated/sklearn.datasets.load_svmlight_file.html#sklearn.datasets.load_svmlight_file> "sklearn.datasets.load_svmlight_file") for the svmlight or libSVM sparse format

  * scikit-learn’s [`load_files`](<../modules/generated/sklearn.datasets.load_files.html#sklearn.datasets.load_files> "sklearn.datasets.load_files") for directories of text files where the name of each directory is the name of each category and each file inside of each directory corresponds to one sample from that category
...
Categorical (or nominal) features stored as strings (common in pandas DataFrames) will need converting to numerical features using [`OneHotEncoder`](<../modules/generated/sklearn.preprocessing.OneHotEncoder.html#sklearn.preprocessing.OneHotEncoder> "sklearn.preprocessing.OneHotEncoder") or [`OrdinalEncoder`](<../modules/generated/sklearn.preprocessing.OrdinalEncoder.html#sklearn.preprocessing.OrdinalEncoder> "sklearn.preprocessing.OrdinalEncoder") or similar. See [Preprocessing data](<../modules/preprocessing.html#preprocessing>).

Note: if you manage your own numerical data it is recommended to use an optimized file format such as HDF5 to reduce data load times. Various libraries such as H5Py, PyTables and pandas provide a Python interface for reading and writing data in that format.
